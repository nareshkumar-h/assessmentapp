...
export class AddRepositoryComponent implements OnInit {
  ...
  createRepo() {
    ...
    this.githubService.createRepo(this.repositoryName).subscribe(
      (res) => {
        console.log(res);
        ...
        let repo = {
          ...
          account: response.owner.login,
          repoName: response.name,
          category: this.project.projectType,
          visibility: response.private ? 'PRIVATE' : 'PUBLIC',
          user: this.project.username,
        };
  ...
  getProjectRepoUrl(): string {
    ...
    let projectName =
      this.projectDetail != null
        ? this.projectDetail.projectPrefix
        : 'yourproject';
    var projectUrl = null;
