...
export class InterviewListComponent implements OnInit {
  ...
  listInterviews() {
    this.interviewService.getInterviews(this.userId).subscribe((res: any) => {
      this.interviews = res;
  ...
  createReport(data) {
    ...
    this.reportData.push({
      label: 'SCHEDULED',
      value: result['SCHEDULED'] ? result['SCHEDULED'].length : 0,
    });
    this.reportData.push({
      label: 'COMPLETED',
      value: result['COMPLETED'] ? result['COMPLETED'].length : 0,
    });
    this.reportData.push({
      label: 'CANCELLED',
      value: result['CANCELLED'] ? result['CANCELLED'].length : 0,
    });
    // this.reportData.push({
    //   label: 'PENDING',
