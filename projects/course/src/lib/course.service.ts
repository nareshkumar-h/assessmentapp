...
export class CourseService {
  ...
  constructor(private http:HttpClient, private authService: AuthService, @Inject(COURSE_CONFIG)private config: Config) {     
  ...
  download(courseId){
    ...
    return this.http.get(url,  {headers:this.getHeaders(), responseType: 'blob' as 'json'});
  }
  ...
  importCourseContent(id,content){
    let userId = this.authService.getLoggedInUserId();
    let url = this.apiUrl + `v1/courses/${id}/import/content?createdBy=${userId}`;    
    return this.http.post(url, content, {headers:this.getHeaders()});
  }
  ...
  importCourseQuestion(id,content){
    let userId = this.authService.getLoggedInUserId();
    let url = this.apiUrl + `v1/courses/${id}/import/questions?createdBy=${userId}`;    
    return this.http.post(url, content, {headers:this.getHeaders()});
  }
