...
export class DashboardComponent implements OnInit {
  ...
  getFeatures(){
    ...
    let featureList = [
      {name:"Courses", link: "../" + this.userId + "/courses", image:"courses.jpg",active:this.authService.hasRole(this.user,["U"])},
      {name:"Projects", link:"../"+ this.userId + "/projects", image:"project.png",active:this.authService.hasRole(this.user, ["U"])},
      {name:"Training", link:"../training/"+ this.userId, image:"training.jpg",active:this.authService.hasRole(this.user,["T"])},
      {name:"Batches", link:"../batches/"+ this.userId, image:"training.jpg",active:this.authService.hasRole(this.user,["T"])}
    ];
...
//    this.menus.push( {title: "Tasks",  path:["/tasks"], icontype:"fas fa-tasks",  access: this.authService.hasRole(this.user,["T"])});
    ...
    this.menus.push( {title: "Trainings",  path:["/training/" +this.userId], icontype:"fas fa-cubes",  access: this.authService.hasRole(this.user,["T"])});
    this.menus.push( {title: "Batches",  path:["/batches"], icontype:"fas fa-cubes",   access: this.authService.hasRole(this.user,["T","HR"])});
    this.menus.push( {title: "Users",  path:["/users"], icontype:"fas fa-users",  access:  this.authService.hasRole(this.user,["T","HR"])});

    this.menus.push( {title: "Courses",  path:["/courses"], icontype:"fas fa-cubes",  access: this.authService.hasRole(this.user,["T"])});
