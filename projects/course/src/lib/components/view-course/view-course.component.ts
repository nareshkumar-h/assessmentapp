...
export class ViewCourseComponent implements OnInit {
  ...
  breadcrumbItems: any = [{ icon: 'home', name: ' Courses', link: '../' }];
  ...
  loadContents(sections) {
    this.ktClient
      .getCourseClient()
      .getCourseContents(this.courseId)
      .then((res) => {
        let data = res;
  ...
  loadModulesMenus(sections) {
    ...
    for (let m of sections) {
      ...
      this.menus.push({
        title: title,
        path: ['/courses', this.courseId, 'modules', m.id, m.title],
        icontype: '',
        access: true,
      });
    }
  ...
  loadMenus() {
    ...
    this.menus.push({
      title: 'Curriculum',
      path: ['/courses', this.courseId, 'curriculum'],
      icontype: 'fas fa-book-open',
      access: true,
    });
    this.menus.push({
      title: 'Topics',
      path: ['/courses', this.courseId, 'topics'],
      icontype: 'fas fa-book-open',
      access: true,
    });
    this.menus.push({
      title: 'Contents',
      path: ['/courses', this.courseId, 'contents'],
      icontype: 'fas fa-book-open',
      access: true,
    });
    this.menus.push({
      title: 'Questions',
      path: ['/courses', this.courseId, 'questions'],
      icontype: 'fas fa-book-open',
      access: true,
    });
