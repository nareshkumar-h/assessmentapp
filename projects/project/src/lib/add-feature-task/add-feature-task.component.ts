...
export class AddFeatureTaskComponent implements OnInit {
  ...
  task = {
    ...
    category: null,
    estimation: 1,
    assignedTo: this.authService.getSelectedUser(),
  };
  ...
  ngOnInit(): void {
    ...
    if (this.taskType == 'BUG') {
      ...
      this.taskCategories = [
        { name: 'DESIGN', description: 'Design Webpage' },
        { name: 'MOCKAPI', description: 'Integrate Mock API' },
        { name: 'RESTAPI', description: 'Develop REST API' },
  ...
  changeAssignTo() {
    this.task.name = this.taskCategories.find(
      (obj) => obj.name == this.task.category
    ).description;
    if (this.taskType == 'BUG') {
