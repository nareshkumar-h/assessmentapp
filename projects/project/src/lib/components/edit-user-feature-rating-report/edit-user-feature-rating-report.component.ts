...
export class EditUserFeatureRatingReportComponent implements OnInit {
  ...
  constructor(private projectService:ProjectService, private toastr:ToastrService, private authService:AuthService, private route:ActivatedRoute, private userService:UserService) { 
  ...
  list(){
    this.projectService.listUserFeatureRatingsForUser(this.userId).subscribe (res=>{
      console.log(res);
      this.ratings = res;
  ...
  displayedColumns: string[] = [ 'id','featureName','functionality','design','bestPractices','timeline','complexity','lastModifiedDate'];
  ...
  updateRating(f){
    ...
    this.projectService.updateFeatureReviewRating(f.featureId, f).subscribe(res=>{
