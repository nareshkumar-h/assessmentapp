...
export class ViewCourseCurriculumComponent implements OnInit {
  ...
  deleteModule(i,module){
    ...
    if(data && data.length> 1 ){
      ...
      if(cfm){
        this.courseService.deleteSection(this.course.id, module.id).subscribe(res=>{
  ...
  hasContents(module){
    let result =  (this.contents[module] && this.contents[module].length>0)?true:false;
  ...
  drop(event: CdkDragDrop<string[]>,moduleName:string) {
    alert("drop-" + moduleName + "-" +  event.previousIndex + "-" + event.currentIndex);
  ...
  download(){
    ...
    this.courseService.download(this.course.id).subscribe (data=>{
      console.log(data);
  ...
  publish(){
    this.courseService.publish(this.course.id).subscribe (res=>{
      console.log(res);
  ...
  loadMenus(){
    ...
    this.menus.push( {title: "Curriculum",  path:["/courses",this.course.id], icon:"fas fa-book-open", access: true});
  ...
  deleteLecture(lecture){
    ...
    if(cfm){
      this.courseService.deleteLecture(this.course.id, lecture.id).subscribe(res=>{
        this.toastr.success("Successfully Deleted");
        this.findContent();
      },
      err=>{
