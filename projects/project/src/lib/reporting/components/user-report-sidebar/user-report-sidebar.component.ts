...
export class UserReportSidebarComponent implements OnInit {
  ...
  navigatePath(path){
    if(path=='features'){
      ...
      this.router.navigate(["reports/" + this.userId + "/features"],{queryParams:{display:'name,description'}});
    }
    else if (path=="technologies"){
      this.router.navigate(["reports/" + this.userId + "/features"],{queryParams:{display:'name,technologies'}});
    }
    else{
      alert(path);
    }
