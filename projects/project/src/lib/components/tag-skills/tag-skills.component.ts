...
@Component({
  selector: 'lib-tag-skills',
  templateUrl: './tag-skills.component.html',
  styleUrls: ['./tag-skills.component.css'],
})
export class TagSkillsComponent implements OnInit {
  @Input()
  featureId: number;
  ...
  removeTag(tag,checked){
    let tagData = { featureId: this.featureId, tag: tag, enabled: false };    
  ...
  addTag(tag, checked){
    let tagData = { featureId: this.featureId, tag: tag, enabled: true };
    this.tags.push(tag);
    this.updateTag(this.featureId, tagData).subscribe(res=>{
  ...
  updateTag(featureId, tagData) {
      return this.projectService
        .updateTag(this.featureId, tagData);
  }
}
