...
export class ProjectActivityListComponent implements OnInit {
  ...
  findOne() {
    this.projectService.findOne(this.projectId).subscribe((res) => {
      this.project = res;
      this.isLoggedInUser =
        this.authService.getLoggedInUsername() == this.project['createdBy'];
    });
  }
  ...
  addActivity() {
    ...
    const dialogRef = this.dialog.open(AddProjectActivityComponent, {
      width: '800px',
      height: 'fit-content',
      data: { projectId: this.projectId, activities: activityNames },
    });
  ...
  deleteActivity(activity) {
    let cfm = confirm(
      'Do you want to delete activity - ' + activity.activityName + '?'
    );
    if (cfm) {
      this.projectService
        .deleteActivity(this.projectId, activity.id)
        .subscribe((res) => {
  ...
  startActivity(activity) {
    let cfm = confirm(
      'Do you want to start activity - ' + activity.activityName + '?'
    );
    if (cfm) {
      let status = 'STARTED';
      this.projectService
        .updateActivityStatus(this.projectId, activity.id, status)
        .subscribe((res) => {
