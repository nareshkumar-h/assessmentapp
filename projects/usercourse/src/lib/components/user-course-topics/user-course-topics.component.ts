...
//            console.log(obj);
  ...
  updateStatus(topic, checked) {
    ...
    if(topic.userTopicId){
       this.userCourseService.updateTopicStatus(topic.userTopicId, status).subscribe (res=>{
        // console.log(res);
  ...
  displayReport(){
    ...
    this.reportData.push({ "label": "Percentage", "value": this.report.getPercentage() + "%" });
  }
  ...
  loadMenus(){
    ...
    this.menus.push( {name: "Topics",  link:["../"+ this.courseId], icon:"fas fa-book-open",  access: true});
